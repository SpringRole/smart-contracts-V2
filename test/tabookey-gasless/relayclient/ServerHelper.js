    ...
    for (; !firstRelayToRespond && this.remainingRelays.length; ) {
      ...
      try {
        ...
        firstRelayToRespond = await this.raceToSuccess(
          slice.map(relay =>
            this.getRelayAddressPing(relay.relayUrl, this.gasPrice)
          )
        );
        if (this.verbose) {
          console.log(
    ...
    return new Promise(function(resolve, reject) {
      let callback = function(error, body) {
        ...
        if (!body || !body.Ready || body.MinGasPrice > gasPrice) {
          reject(
            'Relay not ready or proposed gas price too low ' +
              JSON.stringify(body)
          );
          return;
        }
...
class ServerHelper {
  ...
  async fetchRelaysAdded() {
    ...
    let addedAndRemovedEvents = await this.relayHubInstance.getPastEvents(
      'allEvents',
      {
        fromBlock: fromBlock
    ...
    if (filteredRelays.length == 0) {
      throw new Error(
        'no valid relays. orig relays=' + JSON.stringify(origRelays)
      );
    }
