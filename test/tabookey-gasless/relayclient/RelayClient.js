...
class RelayClient {
  ...
  constructor(web3, config) {
    ...
    this.failedRelays = {};
    this.serverHelper =
      this.config.serverHelper ||
      new ServerHelper(this.httpSend, this.failedRelays, this.config);
  }
  ...
  validateRelayResponse(
    ...
    let signer = ethUtils.bufferToHex(
      ethUtils.pubToAddress(ethUtils.ecrecover(message, tx_v[0], tx_r, tx_s))
    );
    let request_decoded_params = abi_decoder.decodeMethod(returned_tx.input)
      .params;
    ...
    if (
      returned_tx_params_hash === transaction_orig_params_hash &&
      address_relay === signer
    ) {
      ...
      console.error(
        ...
        JSON.stringify(request_decoded_params)
      );
  ...
  sendViaRelay(
    ...
    return new Promise(function(resolve, reject) {
      ...
      let callback = async function(error, body) {
        ...
        if (!body || !body.nonce) {
          reject(
            "Empty body received from server, or neither 'error' nor 'nonce' fields present."
          );
          return;
        }
        ...
        let receivedNonce = validTransaction.nonce.readUIntBE(
          0,
          validTransaction.nonce.byteLength
        );
        if (receivedNonce > relayMaxNonce) {
          ...
          reject(
            'Relay used a tx nonce higher than requested. Requested ' +
              relayMaxNonce +
              ' got ' +
              receivedNonce
          );
      ...
      if (self.config.verbose) {
        ...
        console.log(
          'sendViaRelay to URL: ' +
            relayUrl +
            ' ' +
            JSON.stringify(jsonRequestData, replacer)
        );
      }
  ...
  async relayTransaction(encodedFunctionCall, options) {
    ...
    let gasPrice =
      this.config.force_gasPrice || //forced gasprice
    ...
    for (;;) {
      ...
      let hash = utils.getTransactionHash(
        options.from,
        options.to,
        encodedFunctionCall,
        options.txfee,
      ...
      let signature;
      if (
        typeof self.ephemeralKeypair === 'object' &&
        self.ephemeralKeypair !== null
      ) {
        signature = await getTransactionSignatureWithKey(
          self.ephemeralKeypair.privateKey,
          hash
        );
      } else {
        signature = await getTransactionSignature(
          this.web3,
          options.from,
          hash
        );
      ...
      let relayMaxNonce =
        (await this.web3.eth.getTransactionCount(relayAddress)) +
        allowed_relay_nonce_gap;
  ...
  useKeypairForSigning(ephemeralKeypair) {
    if (ephemeralKeypair && typeof ephemeralKeypair.privateKey === 'string') {
      ephemeralKeypair.privateKey = Buffer.from(
        removeHexPrefix(ephemeralKeypair.privateKey),
        'hex'
      );
    }
